...
export const executeMove = (currentState: GameState, pitIndex: number): GameState => {
  ...
  // --- Normal Distribution Logic ---
  // Note: If totalSeeds == 1 but !isLastPit, we fall through here. 
  // The seed will be moved to the next pit (0 -> 1), waiting to reach the end.
  ...
  if (seeds >= 14) {
    ...
    // Skip start pit logic is implicit in loop count vs seed count.
    // We are now at the point where we distribute into opponent only.
...
// Check for Game Over
const checkWinCondition = (state: GameState, forceEnd: boolean = false): GameState => {
  ...
  const canPlay = currentIndices.some((idx: number) => board[idx] > 0);
